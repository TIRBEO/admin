...
import { Users, UserPlus, UserX, Layers } from 'lucide-react';
...
export default function DirectoryOverview() {
  ...
  const tabs = [
    ...
    { id: 'ous', label: 'Organizational units' },
    { id: 'settings', label: 'Settings' },
  ];

  return (
    <AdminSection title="Directory" description="Manage users, groups, and organizational units"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div className="rounded-xl border border-[var(--color-admin-border)] bg-[var(--color-admin-surface)] p-5">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-medium text-[var(--color-admin-text-secondary)]">Groups</span>
            <Layers className="w-4 h-4 text-[var(--color-warning)]" />
          </div>
          <p className="text-2xl font-semibold text-[var(--color-admin-text)]">—</p>
        </div>
        <div className="rounded-xl border border-[var(--color-admin-border)] bg-[var(--color-admin-surface)] p-5">
      ...
      <div className="rounded-xl border border-[var(--color-admin-border)] bg-[var(--color-admin-surface)] p-6">
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          <button onClick={() => router.push('/admin/directory/users')}
            className="p-4 rounded-lg border border-[var(--color-admin-border)] hover:bg-[var(--color-admin-surface-hover)] transition-colors text-left">
          ...
          <button onClick={() => router.push('/admin/directory/groups')}
            className="p-4 rounded-lg border border-[var(--color-admin-border)] hover:bg-[var(--color-admin-surface-hover)] transition-colors text-left">
            <Layers className="w-5 h-5 text-[var(--color-warning)] mb-2" />
            <p className="text-sm font-medium text-[var(--color-admin-text)]">Manage Groups</p>
            <p className="text-xs text-[var(--color-admin-text-muted)] mt-0.5">Create and manage user groups</p>
          </button>
          <button onClick={() => router.push('/admin/directory/ous')}
            className="p-4 rounded-lg border border-[var(--color-admin-border)] hover:bg-[var(--color-admin-surface-hover)] transition-colors text-left">
            <Layers className="w-5 h-5 text-[var(--color-info)] mb-2" />
            <p className="text-sm font-medium text-[var(--color-admin-text)]">Organizational Units</p>
            <p className="text-xs text-[var(--color-admin-text-muted)] mt-0.5">Structure your organization hierarchy</p>
          </button>
        </div>
      </div>
    </AdminSection>
