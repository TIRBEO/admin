      ...
      <div className="w-9 h-9 rounded-lg bg-gray-50 flex items-center justify-center">
        <Eye className="w-4 h-4 text-gray-500" />
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">All security-related events are logged</p>
          </div>
        </div>
        <div className="space-y-2">
          <div className="flex items-center justify-between py-1">
            <span className="text-xs text-gray-500">Failed Logins (24h)</span>
            <span className="text-xs text-gray-700">{failedLogins !== null ? failedLogins : "..."}</span>
          </div>
          <div className="flex items-center justify-between py-1">
            <span className="text-xs text-gray-500">API Key Usage (7d)</span>
            <span className="text-xs text-gray-700">{apiKeyUsage !== null ? `${apiKeyUsage} requests` : "..."}</span>
          </div>
          {recentEvents.length > 0 && (
            <div className="border-t border-gray-200 pt-3 mt-3 space-y-1.5">
              <p className="text-xs text-gray-500 font-medium">Recent Events</p>
              {recentEvents.map((ev: any) => (
                <div key={ev.id} className="flex items-center justify-between text-xs">
                  <span className="text-gray-500 truncate max-w-[200px]">{ev.action}</span>
      ...
      <div className="rounded-xl border border-gray-200 bg-white overflow-hidden mb-4">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">Manage API keys for external integrations</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-5 border-b border-gray-200">
          <div className="flex gap-3">
            <input
              ...
              className="flex-1 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 placeholder-gray-400 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
            />
        ...
        <div className="divide-y divide-gray-200">
          {loadingKeys ? (
            ...
            <div className="p-6 text-center text-sm text-gray-500">No API keys yet. Create one above.</div>
          ) : apiKeys.map(key => (
              ...
              <div className="flex items-start justify-between mb-2">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${key.is_active ? "bg-emerald-50 text-emerald-700" : "bg-gray-100 text-gray-500"}`}>
                      {key.is_active ? "Active" : "Disabled"}
                    </span>
                  </div>
                  <p className="text-xs text-gray-500 mt-0.5">Created {new Date(key.created_at).toLocaleDateString()}</p>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => rotateKey(key.id)}
                    className="flex items-center gap-1 px-3 py-1.5 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded-lg text-xs transition-colors">
                  ...
                  <button onClick={() => deleteKey(key.id)}
                    className="flex items-center gap-1 px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-700 rounded-lg text-xs transition-colors">
                    <Trash2 className="h-3 w-3" /> Delete
                  </button>
                </div>
              </div>
              <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2 font-mono text-xs text-gray-500 break-all">
                <span>{showNewKey === key.id ? key.key_value : key.prefix + "_••••••••••••••••••••"}</span>
                <button onClick={() => setShowNewKey(showNewKey === key.id ? null : key.id)} className="shrink-0 text-gray-500 hover:text-gray-700">
                  <Eye className="h-3.5 w-3.5" />
                </button>
                <button onClick={() => copyToClipboard(key.key_value, key.id)} className="shrink-0 text-gray-500 hover:text-gray-700">
                  {copiedKey === key.id ? <Check className="h-3.5 w-3.5 text-emerald-400" /> : <Copy className="h-3.5 w-3.5" />}
                </button>
              </div>
              {key.last_used_at && <p className="text-xs text-gray-400 mt-1">Last used: {new Date(key.last_used_at).toLocaleString()}</p>}
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="p-5 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Session, MFA, password, and verification settings</p>
            </div>
          </div>
        </div>

        {loadingAuth ? (
          <div className="p-6 text-center text-sm text-gray-500">Loading auth settings...</div>
        ) : (
          <div className="p-5 space-y-5">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500">Maximum session length before re-authentication</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <span className="text-sm text-gray-700">Min Password Length</span>
              ...
              <input type="number" min={4} max={64} value={authSettings.password_min_length}
                ...
                className="w-24 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 text-center" />
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500">Failed attempts before temporary lockout</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-start gap-3">
                <Fingerprint className="w-4 h-4 text-gray-500 mt-0.5" />
                <div>
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                ...
                <div className="w-9 h-5 bg-gray-300 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-blue-600" />
              </label>
            </div>
            ...
            {authSettings.mfa_enabled && (
              <div className="pl-7 space-y-3">
                <p className="text-xs text-gray-500">Allowed MFA Methods</p>
                {[
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Collect and verify phone numbers during signup</p>
                </div>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-start gap-3">
                <Smartphone className="w-4 h-4 text-gray-500 mt-0.5" />
                <div>
                  <span className="text-sm text-gray-700">6-Digit OTP Verification</span>
                  <p className="text-xs text-gray-500">Use 6-digit codes instead of magic links for email verification</p>
                </div>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
      ...
      <div className="rounded-xl border border-gray-200 bg-white overflow-hidden mb-4">
        <div className="p-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="text-sm font-semibold text-gray-800">CORS & Domains</h3>
              <p className="text-xs text-gray-500">Allowed origins and redirect URLs</p>
            </div>
          </div>
          <div className="space-y-2">
            <div className="flex items-center justify-between py-1">
              <span className="text-xs text-gray-500">Allowed Origins</span>
              <span className="text-xs text-gray-700">3 domains</span>
            </div>
            <div className="flex items-center justify-between py-1">
              <span className="text-xs text-gray-500">Redirect URLs</span>
              <span className="text-xs text-gray-700">5 URLs configured</span>
            </div>
          </div>
        </div>
      </div>
