  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => { setEditing({ label: "", href: "", sort_order: items.length, is_active: true }); setShowForm(true); }} className="flex items-center gap-2 rounded-lg bg-neutral-100 px-4 py-2 text-sm font-medium text-neutral-900 hover:bg-neutral-200 transition-colors">
      ...
      {showForm && (
        <div className="mb-6 rounded-xl border border-neutral-800 bg-neutral-900/50 p-4 space-y-3">
          <input placeholder="Label" value={editing.label ?? ""} onChange={(e) => setEditing({ ...editing, label: e.target.value })} className="w-full rounded-lg border border-neutral-800 bg-neutral-950 px-3 py-2 text-sm" />
          <input placeholder="Href" value={editing.href ?? ""} onChange={(e) => setEditing({ ...editing, href: e.target.value })} className="w-full rounded-lg border border-neutral-800 bg-neutral-950 px-3 py-2 text-sm" />
      ...
      <div className="space-y-1">
        {items.map((item) => (
          <div key={item.id} className="flex items-center gap-3 rounded-lg border border-neutral-800 bg-neutral-900/30 px-4 py-3 text-sm">
            <GripVertical className="h-4 w-4 text-neutral-600 shrink-0" />
            <span className="flex-1">{item.label}</span>
            <span className="text-neutral-500 text-xs">{item.href}</span>
            <span className={`text-xs px-2 py-0.5 rounded-full ${item.is_active ? "bg-green-500/10 text-green-400" : "bg-neutral-800 text-neutral-500"}`}>{item.is_active ? "Active" : "Inactive"}</span>
