...
const AVAILABLE_INTEGRATIONS = [
  ...
  { name: "GitHub", type: "github", icon: "Code", desc: "Track issues, PRs, and repository events", color: "bg-neutral-800 text-neutral-300", config: [{ key: "token", label: "Personal Access Token", type: "password", placeholder: "ghp_..." }, { key: "repo", label: "Default Repository", type: "text", placeholder: "owner/repo" }] },
  ...
  { name: "Resend", type: "resend", icon: "Mail", desc: "Email delivery via Resend API", color: "bg-white/10 text-white", config: [{ key: "api_key", label: "API Key", type: "password", placeholder: "re_..." }, { key: "from", label: "From Address", type: "email", placeholder: "noreply@tirbeo.com" }] },
  ...
  { name: "Vercel", type: "vercel", icon: "Cloud", desc: "Deployments and environment management", color: "bg-white/10 text-white", config: [{ key: "token", label: "API Token", type: "password", placeholder: "vcp_..." }, { key: "team_id", label: "Team ID", type: "text", placeholder: "Optional" }] },
  ...
  { name: "Twitter/X", type: "twitter", icon: "MessageSquare", desc: "Social media posting and analytics", color: "bg-neutral-800 text-neutral-300", config: [{ key: "bearer_token", label: "Bearer Token", type: "password" }, { key: "api_key", label: "API Key", type: "password" }, { key: "api_secret", label: "API Secret", type: "password" }] },
  ...
  { name: "Logtail", type: "logtail", icon: "Database", desc: "Centralized logging and observability", color: "bg-neutral-800 text-neutral-300", config: [{ key: "source_token", label: "Source Token", type: "password" }] },
  ...
  return (
    <div className="p-6 max-w-5xl">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="text-2xl font-semibold tracking-tight text-white">{app.name} Integrations</h1>
      </div>

      {/* Config Modal */}
      {configuring && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-neutral-900 rounded-2xl border border-neutral-800 w-full max-w-lg max-h-[80vh] overflow-y-auto m-4 shadow-lg">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-semibold text-white">Configure {configuring.name}</h2>
                <button onClick={() => setConfiguring(null)} className="text-neutral-400 hover:text-neutral-300">
                  <X className="w-5 h-5" />
                </button>
              </div>
                ...
                {AVAILABLE_INTEGRATIONS.find(i => i.type === configuring.type)?.config.map(field => (
                  <div key={field.key}>
                    ...
                    <input
                      ...
                      className="w-full bg-neutral-950 border border-neutral-700 rounded-lg px-3 py-2 text-sm text-white placeholder-neutral-600 font-mono"
                    />
                  </div>
                ))}
              </div>
              <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-neutral-800">
                <button onClick={() => setConfiguring(null)} className="rounded-lg border border-neutral-700 bg-neutral-950 px-4 py-2 text-sm text-neutral-300 hover:bg-neutral-800">Cancel</button>
      ...
      {loading ? (
        <div className="space-y-3">{[1, 2, 3].map(i => <div key={i} className="h-16 rounded-xl bg-neutral-800 border border-neutral-800" />)}</div>
      ) : (
          ...
          <section>
            ...
            {connected.length === 0 ? (
              <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-6 text-center">
                <Plug className="w-8 h-8 mx-auto mb-2 text-neutral-500" />
                <p className="text-sm text-neutral-400">No integrations connected yet</p>
                <p className="text-xs text-neutral-500 mt-1">Add an integration from the marketplace below</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {connected.map(integration => {
                  ...
                  return (
                    <div key={integration.id} className="flex items-center justify-between rounded-xl border border-emerald-800 bg-emerald-500/10 p-4">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-neutral-400 truncate">{tmpl?.desc}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <button onClick={() => openConfig(integration)}
                          className="p-1.5 text-neutral-400 hover:text-white rounded-lg hover:bg-neutral-800 transition-colors" title="Configure">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {disconnected.map(integration => {
                  ...
                  const IconComp = ICONS[tmpl?.icon || ""] || Plug;
                  return (
                    <div key={integration.id} className="flex items-center justify-between rounded-xl border border-neutral-800 bg-neutral-900 p-4">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`w-9 h-9 rounded-xl ${tmpl?.color || "bg-neutral-800"} flex items-center justify-center shrink-0`}>
                          <IconComp className="w-4 h-4" />
                        </div>
                ...
                return (
                  <div key={tmpl.type}
                    className={`rounded-xl border p-4 transition-all ${
                      installed
                        ? "border-emerald-800 bg-emerald-500/10 opacity-50"
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-neutral-400 mt-0.5">{tmpl.desc}</p>
                      </div>
                    </div>
                    {installed ? (
                      ...
                      <button onClick={() => addIntegration(tmpl)}
                        className="mt-3 w-full px-3 py-1.5 bg-neutral-800 hover:bg-blue-600 text-neutral-300 hover:text-white rounded-lg text-xs transition-colors">
                        Add Integration
                      </button>
                    )}
                  </div>
