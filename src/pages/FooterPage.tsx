  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="space-y-4">
        {sections.map((sec) => (
          <div key={sec.id} className="rounded-lg border border-neutral-800 bg-neutral-900 p-4">
            <div className="flex items-center gap-3 mb-3">
              <span className="font-medium text-sm">{sec.title}</span>
              <button onClick={() => addLink(sec.id)} className="text-xs text-neutral-500 hover:text-neutral-300 ml-auto"><Plus className="h-3 w-3 inline" /> Add Link</button>
              <button onClick={() => removeSection(sec.id)} className="p-1 text-neutral-500 hover:text-red-600"><Trash2 className="h-3.5 w-3.5" /></button>
            </div>
            <div className="space-y-1">
              {sec.links.map((link) => (
                <div key={link.id} className="flex items-center gap-3 rounded bg-neutral-800 px-3 py-2 text-xs text-neutral-500">
