  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="space-y-2">
        {pages.map((page) => (
          <div key={page.id} className="rounded-lg border border-neutral-800 bg-neutral-900">
            <button onClick={() => togglePage(page.slug)} className="flex w-full items-center gap-3 px-4 py-3 text-left text-sm">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full ${page.is_published ? "bg-green-50 text-green-700" : "bg-neutral-800 text-neutral-500"}`}>{page.is_published ? "Published" : "Draft"}</span>
            </button>

            {expanded === page.slug && (
              <div className="border-t border-neutral-800 px-4 pb-4 pt-3 space-y-2">
                {page.sections?.map((section) => (
                  <div key={section.id} className="rounded-lg bg-neutral-800 px-3 py-2">
                    <div className="flex items-center gap-3 text-sm">
                      ...
                      {section.type === "features" && (
                        <button onClick={() => { setEditingFeature({ section_id: section.id, title: "", sort_order: 0, is_active: true }); setShowFeatureForm(true); loadFeatures(section.id); }} className="text-xs text-neutral-500 hover:text-neutral-300">Manage Features</button>
                      )}
                      <button onClick={() => { setEditingSection(section); setShowSectionForm(true); }} className="p-1 text-neutral-500 hover:text-neutral-300"><Pencil className="h-3 w-3" /></button>
                      <button onClick={() => removeSection(section.id)} className="p-1 text-neutral-500 hover:text-red-600"><Trash2 className="h-3 w-3" /></button>
                    </div>

                    {showFeatureForm && editingFeature.section_id === section.id && (
                      <div className="mt-3 space-y-2 border-t border-neutral-800 pt-3">
                        {features.map((f) => (
                          <div key={f.id} className="flex items-center gap-3 text-xs text-neutral-500">
                            <span className="flex-1">{f.title}</span>
                            <button onClick={() => { setEditingFeature(f); }} className="p-1 hover:text-neutral-300"><Pencil className="h-3 w-3" /></button>
                            <button onClick={() => removeFeature(f.id)} className="p-1 hover:text-red-400"><Trash2 className="h-3 w-3" /></button>
                          </div>
                        ))}
                        <div className="flex gap-2 pt-2">
                          <input placeholder="Title" value={editingFeature.title ?? ""} onChange={(e) => setEditingFeature({ ...editingFeature, title: e.target.value })} className="flex-1 rounded border border-neutral-700 bg-neutral-950 px-2 py-1 text-xs text-white" />
                          <input placeholder="Description" value={editingFeature.description ?? ""} onChange={(e) => setEditingFeature({ ...editingFeature, description: e.target.value })} className="flex-1 rounded border border-neutral-700 bg-neutral-950 px-2 py-1 text-xs text-white" />
                ...
                <button onClick={() => { setEditingSection({ page_slug: page.slug, type: "hero", title: "", sort_order: (page.sections?.length || 0), is_active: true }); setShowSectionForm(true); }}
                  className="flex items-center gap-2 text-xs text-neutral-500 hover:text-neutral-300 mt-2">
