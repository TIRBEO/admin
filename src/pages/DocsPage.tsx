              ...
              {canWrite && editingCategory !== cat.id && (
                <div className="flex items-center gap-0.5">
                  <button onClick={() => moveCategory(cat.id, "up")} disabled={reordering}
                    className="p-1 text-neutral-500 hover:text-neutral-200 disabled:opacity-30"><ArrowUp className="h-3.5 w-3.5" /></button>
                  <button onClick={() => moveCategory(cat.id, "down")} disabled={reordering}
                    className="p-1 text-neutral-500 hover:text-neutral-200 disabled:opacity-30"><ArrowDown className="h-3.5 w-3.5" /></button>
                </div>
              )}
              {canWrite && editingCategory !== cat.id && (
                        ...
                        {canWrite && (
                          <div className="flex items-center gap-0.5">
                            <button onClick={() => moveArticle(cat.id, article.id, "up")} disabled={reordering}
                              className="p-0.5 text-neutral-500 hover:text-neutral-200 disabled:opacity-30"><ArrowUp className="h-3 w-3" /></button>
                            <button onClick={() => moveArticle(cat.id, article.id, "down")} disabled={reordering}
                              className="p-0.5 text-neutral-500 hover:text-neutral-200 disabled:opacity-30"><ArrowDown className="h-3 w-3" /></button>
                          </div>
                        )}
