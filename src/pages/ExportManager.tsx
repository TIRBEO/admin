    ...
    <div className="max-w-4xl">
      ...
      <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-6 mb-6">
        ...
        <div className="grid grid-cols-2 gap-6">
          <div>
            <label className="block text-sm text-neutral-500 mb-1.5">Data Type</label>
            <select value={exportType} onChange={(e) => setExportType(e.target.value)} className="w-full rounded-lg border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm text-white">
          ...
          <div>
            <label className="block text-sm text-neutral-500 mb-1.5">Format</label>
            <select value={format} onChange={(e) => setFormat(e.target.value)} className="w-full rounded-lg border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm text-white">
          ...
          <div>
            <label className="block text-sm text-neutral-500 mb-1.5">Schedule</label>
            <select value={schedule} onChange={(e) => setSchedule(e.target.value)} className="w-full rounded-lg border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm text-white">
      ...
      <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-6">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-medium text-neutral-500">Recent Exports</h3>
        ...
        <div className="space-y-2">
          {loading ? (
            ...
            return (
              <div key={exp.id} className="flex items-center justify-between rounded-lg bg-neutral-900 p-3">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-lg bg-neutral-800"><Icon className="h-4 w-4 text-neutral-500" /></div>
                  <div>
                ...
                <div className="flex items-center gap-2">
                  {exp.status === "processing" ? (
                    ...
                    <>
                      {exp.file_url && (
                        <button onClick={() => downloadExport(exp)} className="text-neutral-500 hover:text-white transition-colors" title="Download">
